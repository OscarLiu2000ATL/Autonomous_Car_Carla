...
IM_WIDTH = 640
IM_HEIGHT = 480
thrott = [["frame", "throttle", "steer", "brake"]]
...
def main():
    ...
    try:
        ...
        # --------------
        '''
        client.start_recorder("./recordings00.log")
        '''
        ...
        ego_cam.listen(lambda image: image.save_to_disk('./outputs09/%.6d.png' % image.frame))
        ...
        while True:
            ...
            print(frame)
            if(frame > old_timestamp):
	            thrott.append([frame, ego_vehicle.get_control().throttle,
	            				ego_vehicle.get_control().steer, ego_vehicle.get_control().brake])
	            old_timestamp = frame
        ...
        with open('data09.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerows(thrott)
